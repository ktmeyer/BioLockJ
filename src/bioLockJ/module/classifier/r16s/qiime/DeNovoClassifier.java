 ...
 * as a single batch.
 */
public class DeNovoClassifier extends QiimeClassifier
{
	private boolean removeChimeras = false;
	private final String repSet = REP_SET + File.separator + "*.fasta";
	...
	{
		BashScriptUtil.buildScripts( this, buildScript( getInputFiles() ), "QIIME.DeNovoClassifier" );
	}

	/**
	 * Dir top level holds only fasta files and it's sub-directory holds the corrected
	...
	protected List<List<String>> buildScript( final List<File> files ) throws Exception
	{
		...
		if( removeChimeras )
		{
			...
			final String line2 = SCRIPT_FILTER_OTUS + " otu_table.biom -e " + otusToFilter + " -o " + outputDir
					+ OTU_TABLE;
