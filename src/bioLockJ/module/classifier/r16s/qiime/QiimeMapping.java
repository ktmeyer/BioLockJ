...
public class QiimeMapping extends bioLockJ.BioLockJ
{
	...
	/**
	 * Add any alpha metrics from previous metadata that are not included in new metadata.
	 * Output keeps QIIME_ID as key
	 * @param cReader
	 * @param outputDir
	 * @param oldMeta
	 * @return
	 * @throws Exception
	 */
	public static void mergeAlphaMetrics( final File outputDir, final File oldMeta ) throws Exception
	{
		log.info( "Calling mergeAlphaMetrics()" );
		...
		try
		{
			...
			for( String line = reader.readLine(); line != null; line = reader.readLine() )
			{
				...
				final String restOfLine = line.substring( id.length() );
				if( isHeaderRow )
				{
					map = getAlphaMetricsMap( oldMeta, restOfLine );
					if( map == null )
					{
						return;
					}
					log.info( "Output Alpha Metrics to: " + newMeta.getAbsolutePath() );
					isHeaderRow = false;
					fileNameCol = getFileNameColumn( restOfLine );
					newMeta = new File( outputDir.getAbsolutePath() + File.separator
							+ config.getMetaUtil().getMetadata().getName() );
					writer = new BufferedWriter( new FileWriter( newMeta ) );
					config.getMetaUtil().updateDescriptor( map.get( id ), outputDir );
					sb.append( id ).append( restOfLine );
				}
				else
				{
					final String sampleId = getSampleIdFromMappingFile( id, fileNameCol );
					sb.append( id ).append( restOfLine );
					id = sampleId;
				}

				for( final String alphaMetric: map.get( id ) )
				{
					sb.append( DELIM ).append( alphaMetric );
				}
		...
		{
			log.error( "Error occurred merging old alpha metrics into new metadata file: ", ex );
		}
		finally
		{
			reader.close();
			...
			config.setMetadata( newMeta );
			log.info( "Updated metadata file to: " + newMeta );
		}
	}
